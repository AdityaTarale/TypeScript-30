...
const fifteen = inventers.filter((inventer) => {
    return inventer.year >= 1500 && inventer.year <= 1600;
});
console.table(fifteen);
// Array.prototype.map()
// 2. Give us an array of the inventors first and last names
const fullName = inventers.map((inventer) => {
    return `${inventer.first} ${inventer.last}`;
});
console.table(fullName);
// Array.prototype.sort()
// 3. Sort the inventors by birth date, oldest to youngest
const ordered = inventers.sort((a, b) => {
    return a.year < b.year ? -1 : 1;
});
console.table(ordered);
// Array.prototype.reduce()
// 4. How many years did all the inventors live all together?
const totalYears = inventers.reduce((years, inventer) => years + (inventer.passed - inventer.year), 0);
console.log(totalYears);
// 5. Sort the inventors by years lived
const oldest = inventers.sort((a, b) => {
